...
export default function ReservarClient() {
  ...
  // Fechas y huéspedes: antes venían SOLO de la URL (?checkin=&checkout=)
  // y esta página los mostraba como texto fijo ("Por elegir"), sin ninguna
  // forma de completarlos o corregirlos acá. Bug reportado por Andre
  // (7/9/2026): "no me pidió fechas... donde no me dejó modificar". Ahora
  // son inputs editables, precargados desde la URL cuando kuhane-web los
  // manda, pero siempre modificables desde esta misma página.
  ...
  // No se puede avanzar sin fechas válidas — pedido explícito de Andre:
  // "no debería dejarme avanzar si no lleno los campos de fechas y
  // cantidad de personas". La cantidad de huéspedes siempre tiene un valor
  // (mínimo 1), así que lo que falta validar son las fechas.
  const datesIncomplete = !checkin || !checkout || !nightCount;
  ...
  async function submitRequest() {
    if (!selectedRoomId || datesIncomplete || missingRequired) return;
    setSending(true);
    setError(null);
    ...
    <main className="min-h-screen bg-paper-alt px-6 py-16 sm:px-10">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-8 rounded-xl border border-line bg-surface p-5">
          <div
            className={`grid grid-cols-1 gap-4 sm:grid-cols-3 ${promo ? "sm:grid-cols-4" : ""}`}
          >
            <div>
              <label className="font-mono-ui text-[11px] uppercase tracking-widest text-ink-faint">
                Llegada *
              </label>
              <input
                type="date"
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-3 rounded-lg border border-line bg-paper px-3 py-2">
                <button
                  type="button"
                  aria-label="Menos huéspedes"
                  onClick={() => setGuests((g) => Math.max(1, g - 1))}
                  className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-line text-ink-soft hover:bg-surface"
                >
                  −
                </button>
          ...
          {nightCount ? (
            ...
            <p className="mt-3 text-xs text-rust">
              Elige llegada y salida para ver las habitaciones disponibles.
            </p>
          )}
        </div>
          ...
          {displayRooms.map((room) => (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  disabled={!room.real || datesIncomplete}
                  ...
                  title={datesIncomplete ? "Elige llegada y salida primero" : undefined}
                >
